...
# Task: GMT-only, no FrontRES-specific observations
...
def _build_env(motion_file: str, num_envs: int):
    """
    ...
    # Point to our motion file
    env_cfg.commands.motion.motion_file = motion_file
    env_cfg.commands.motion.motion      = motion_file
    ...
    if hasattr(env_cfg.commands.motion, "resampling_time_range"):
        env_cfg.commands.motion.resampling_time_range = (1e9, 1e9)
...
def main() -> None:
    ...
    print(f"\n{'='*60}")
    print(f"  Robustness Budget Validation")
    print(f"  Motion:     {args_cli.motion}")
    print(f"  Checkpoint: {args_cli.checkpoint}")
    print(f"  Epsilons:   {EPSILON_VALUES}")
    print(f"  Push Δv:    {PUSH_VELOCITIES} m/s")
    print(f"  Trials:     {num_envs} (parallel envs)")
    ...
    print(f"[Validation] left_ankle_idx={left_foot_idx}, right_ankle_idx={right_foot_idx}")
